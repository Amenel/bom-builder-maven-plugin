...
public class PomDependencyVersionsTransformerTest {
	...
	public void testDependencyVersionsAreSpecifiedByProperties() throws Exception {
		...
		assertEquals("${" + versionKey + "}",
				transformedModel.getDependencyManagement().getDependencies().get(0).getVersion());
	}
	...
	public void testDependencyVersionIsSpecifiedByPropertyWithKeyIncludingArtifactId() throws Exception {
		...
		assertEquals("${" + versionKey2 + "}",
				transformedModel.getDependencyManagement().getDependencies().get(1).getVersion());
	}

	// FIXME one groupId with same versions, but configuration requires property for
	// given artifactId
